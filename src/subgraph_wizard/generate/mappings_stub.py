...
def render_all_mappings_stub(
    ...
    if config.complexity == "advanced" and config.templates:
        for template in config.templates:
            ...
            result[template.name] = render_template_mapping_stub(
                template, config.network, abi
            )
        logger.info(f"Rendered stub mappings for {len(config.templates)} template(s)")
    
    return result


def render_template_mapping_stub(
    template: TemplateConfig,
    network: str,
    abi: list[dict[str, Any]] | None = None,
) -> str:
    """Render a stub mapping file for a template.
    ...
    Returns:
        Rendered mapping file content as a string.
    """
    logger.info(f"Rendering stub mapping for template: {template.name}")
    ...
    if abi:
        ...
        if events:
            ...
            if filtered_events:
                ...
                handlers = [
                    {
                        "name": get_handler_name(event["name"]),
                        "event_name": event["name"],
                        "event_type": f"{event['name']}Event",
                        "entity_name": get_entity_name(event["name"]),
                        "params": [

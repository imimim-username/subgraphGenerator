...
async def _generate_graph(
    ...
    # .npmrc — enforce lockfile + exact pins for any packages the user adds
    (output_dir / ".npmrc").write_text(_render_graph_npmrc(), encoding="utf-8")
    written.append(str(output_dir / ".npmrc"))
...
def _generate_ponder(
    ...
    # .npmrc — enforce exact pins for any packages the user adds after generation
    (output_dir / ".npmrc").write_text(render_ponder_npmrc(), encoding="utf-8")
    written.append(str(output_dir / ".npmrc"))
...
def _render_package_json(subgraph_name: str) -> str:
    ...
    Dependency versions are pinned to exact known-good releases for
    supply-chain safety (see package_json.py for the pinned constants).
    """
    ...
    pkg = {
        ...
        "dependencies": {
            "@graphprotocol/graph-cli": DEFAULT_GRAPH_CLI_VERSION,
            "@graphprotocol/graph-ts": DEFAULT_GRAPH_TS_VERSION,
        },
    }
    return json.dumps(pkg, indent=2, ensure_ascii=False)


def _render_graph_npmrc() -> str:
    """Return a .npmrc for generated The Graph projects (npm).

    Enforces lockfile generation and exact version pinning so that any
    package the user adds is pinned as "1.2.3" rather than "^1.2.3",
    and the lockfile cannot be accidentally skipped.
    """
    return (
        "# Enforce lockfile and exact version pins — supply-chain hardening.\n"
        "# Generated by subgraph-wizard. Do not remove.\n"
        "package-lock=true\n"
        "save-exact=true\n"
    )
...
> **Supply-chain tip:** commit `package-lock.json` to your repository after this
...
> ```bash
> git add package-lock.json && git commit -m "chore: add npm lockfile"
> ```
